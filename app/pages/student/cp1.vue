  ...
  <div class="min-h-screen bg-gray-100 dark:bg-slate-700 py-8 px-4  print:bg-white print:py-0 print:px-0">
    ...
    <div class="max-w-[210mm] mx-auto mb-6 flex flex-col gap-4 print:hidden">
      <div class="flex flex-col md:flex-row justify-between items-center gap-4">
        <NuxtLink to="/student"
          class="flex items-center gap-2 text-gray-600 dark:text-slate-400 hover:text-[#1a1a40] transition-colors font-medium bg-white dark:bg-slate-800 px-4 py-2 rounded-full shadow-sm w-full md:w-auto justify-center">
          <span class="material-symbols-rounded">arrow_back</span> กลับหน้าหลัก
        </NuxtLink>

        <div class="flex flex-wrap justify-center gap-2 md:gap-3">
          <button v-if="!isAdmin && !isSubmitted" @click="handleSubmit" :disabled="submitting"
            class="bg-blue-600 text-white px-4 md:px-6 py-2 rounded-full font-medium hover:bg-blue-700 transition-colors shadow-sm flex items-center gap-2 text-sm md:text-base disabled:opacity-50">
            <span class="material-symbols-rounded">send</span> {{ submitting ? 'กำลังบันทึก...' : 'ยื่นขออนุมัติหัวข้อ'
            }}
          </button>
          <button @click="printDocument"
            class="bg-[#1a1a40] text-white px-4 md:px-6 py-2 rounded-full font-medium hover:bg-[#2a2a5c] transition-colors shadow-sm flex items-center gap-2 text-sm md:text-base">
      ...
      <div v-if="!isAdmin && !isSubmitted"
        class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          <label
            class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">เลือกอาจารย์ที่ปรึกษาหลัก</label>
          <select v-model="advisorId"
            class="w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 outline-none focus:border-[#1a1a40] transition-all">
            <option value="">-- กรุณาเลือกอาจารย์ --</option>
            <option v-for="t in teachers" :key="t.id" :value="t.id">{{ t.fullname }}</option>
          </select>
        </div>
        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">เลือกอาจารย์ที่ปรึกษาร่วม
            (ถ้ามี)</label>
          <select v-model="coAdvisorId"
            class="w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 outline-none focus:border-[#1a1a40] transition-all">
        ...
        <div>
          <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">เลือกเพื่อนร่วมกลุ่ม
            (ถ้ามี)</label>
          <select v-model="selectedStudentId"
            class="w-full bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 transition-all font-bold">
            <option value="">-- ทำโครงงานคนเดียว --</option>
            <option v-for="s in students" :key="s.id" :value="s.id">{{ s.fullname }} ({{ s.username }})</option>
          </select>
        </div>
      </div>
      <div v-else-if="isSubmitted && !isAdmin"
        class="bg-white dark:bg-slate-800 p-8 rounded-[32px] border border-gray-100 dark:border-slate-700 shadow-sm animate-[fadeIn_0.5s_ease-out]">
        <div class="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 dark:border-slate-700">
          ...
          <div :class="project?.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'"
            class="px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-2">
        ...
        <div class="space-y-4">
          <div
            class="flex flex-col md:flex-row md:items-center gap-6 p-6 rounded-2xl bg-gray-50/50 border border-gray-100 dark:border-slate-700">
            <div
              class="w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 text-indigo-600 shadow-sm flex items-center justify-center text-2xl shrink-0">
              <span class="material-symbols-rounded">title</span>
            </div>
            <div class="flex-grow">
              <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">ชื่อโครงงาน (ภาษาไทย)
              </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              class="flex items-center gap-4 p-5 rounded-2xl bg-gray-50/50 border border-gray-100 dark:border-slate-700">
              <div
                class="w-12 h-12 rounded-xl bg-white dark:bg-slate-800 text-blue-600 shadow-sm flex items-center justify-center text-xl shrink-0">
                <span class="material-symbols-rounded">person_pin</span>
              </div>
              <div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">อาจารย์ที่ปรึกษาหลัก
                </div>
            ...
            <div
              class="flex items-center gap-4 p-5 rounded-2xl bg-gray-50/50 border border-gray-100 dark:border-slate-700">
              <div
                class="w-12 h-12 rounded-xl bg-white dark:bg-slate-800 text-emerald-600 shadow-sm flex items-center justify-center text-xl shrink-0">
                <span class="material-symbols-rounded">group</span>
              </div>
              <div>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">อาจารย์ที่ปรึกษาร่วม
                </div>
          ...
          <div
            class="flex flex-col md:flex-row md:items-center gap-6 p-6 rounded-2xl bg-gray-50/50 border border-gray-100 dark:border-slate-700">
            <div
              class="w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 text-amber-600 shadow-sm flex items-center justify-center text-2xl shrink-0">
              <span class="material-symbols-rounded">history</span>
            </div>
            <div class="flex-grow">
              <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">ข้อมูลการบันทึกล่าสุด
              </div>
              <div class="text-sm font-bold text-[#1a1a40] dark:text-white">แก้ไขเมื่อ: {{ currentDate }}</div>
            </div>
            <button @click="printDocument"
              class="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#1a1a40] text-white font-bold text-sm hover:bg-indigo-600 transition-all shadow-md">
      ...
      <div class="flex flex-col items-center gap-8 min-w-[210mm] print:block print:w-full print:gap-0"
        ...
        <div class="mb-4">
          ...
          <div class="flex items-end mb-1.5 gap-2 w-full relative">
            ...
            <input type="text" v-model="s1.zipcode" @input="handleZipcodeChange(1)"
              class="form-dotted w-24 min-w-0 text-center bg-[#fff9db] font-bold print:bg-transparent">
            <!-- Suggestions Dropdown for S1 -->
            <div v-if="suggestions1.length > 0"
              class="absolute z-50 left-24 top-8 w-64 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl max-h-48 overflow-y-auto print:hidden">
              <div v-for="(s, idx) in suggestions1" :key="idx" @click="selectAddress(1, s)"
            ...
            <input type="text" v-model="s1.tel" class="form-dotted w-32 min-w-0 text-center">
          ...
          <div class="flex items-end mb-1.5 gap-2 w-full">
            ...
            <input type="text" v-model="s1.lineId" class="form-dotted w-48 min-w-0 text-center">
          </div>
        ...
        <div class="mb-6">
          ...
          <div class="flex items-end mb-1.5 gap-2 w-full">
            <span class="shrink-0">ชื่อนักศึกษา (นาย/นางสาว)</span>
            <input type="text" v-model="s2.fullname" class="form-dotted flex-1 min-w-0 text-center"
              placeholder="ปล่อยว่างหากทำคนเดียว">
            <span class="shrink-0">รหัสนักศึกษา</span>
          ...
          <div class="flex items-end mb-1.5 gap-2 w-full relative">
            <span class="shrink-0">รหัสไปรษณีย์</span>
            <input type="text" v-model="s2.zipcode" @input="handleZipcodeChange(2)"
              class="form-dotted w-24 min-w-0 text-center bg-[#fff9db] font-bold print:bg-transparent">
            <!-- Suggestions Dropdown for S2 -->
            <div v-if="suggestions2.length > 0"
              class="absolute z-50 left-24 top-8 w-64 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-xl max-h-48 overflow-y-auto print:hidden">
              <div v-for="(s, idx) in suggestions2" :key="idx" @click="selectAddress(2, s)"
                class="p-2 hover:bg-gray-100 cursor-pointer text-xs border-b border-gray-100 dark:border-slate-700 last:border-0">
                ต.{{ s.subdistrict }} อ.{{ s.district }} จ.{{ s.province }} {{ s.zipcode }}
              </div>
            </div>
            <span class="shrink-0">โทรศัพท์มือถือ</span>
            <input type="text" v-model="s2.tel" class="form-dotted w-32 min-w-0 text-center">
            <span class="shrink-0">โทรศัพท์บ้าน</span>
          ...
          <div class="flex items-end mb-1.5 gap-2 w-full">
            ...
            <span class="shrink-0">Line Id</span>
            <input type="text" v-model="s2.lineId" class="form-dotted w-48 min-w-0 text-center">
          </div>
        ...
        <div class="flex justify-between px-8 mb-5 mt-6 w-full break-inside-avoid">
          <div class="flex flex-col items-center min-w-[250px]">
            ...
            <div class="flex items-end gap-1 w-full justify-center">
              ...
              <span class="form-dotted min-w-[150px] px-2 text-center text-gray-700 dark:text-slate-300">{{ s1.fullname
              }}</span>
              <span class="shrink-0">)</span>
            </div>
          </div>
          ...
          <div class="flex flex-col items-center min-w-[250px]">
            ...
            <div class="flex items-end gap-1 w-full justify-center">
              <span class="shrink-0">(</span>
              <span class="form-dotted min-w-[150px] px-2 text-center text-gray-700 dark:text-slate-300">{{ s2.fullname
              }}</span>
              <span class="shrink-0">)</span>
            </div>
          </div>
        ...
        <div class="flex justify-center items-end gap-2 mb-6 w-full break-inside-avoid">
          ...
          <input type="text" :value="currentDate"
            class="form-dotted w-[180px] min-w-0 text-center text-gray-600 dark:text-slate-400" readonly>
        </div>
        ...
        <div class="flex justify-between px-4 w-full break-inside-avoid">
          <div class="flex flex-col items-center min-w-[280px] mx-5">
            ...
            <div class="flex items-end gap-1 w-full justify-center">
              <span class="shrink-0">(</span>
              <span class="form-dotted min-w-[180px] px-2 text-center text-gray-700 dark:text-slate-300">{{ advisorName
              }}</span>
              <span class="shrink-0">)</span>
            </div>
          </div>
          ...
          <div class="flex flex-col items-center min-w-[280px]">
            ...
            <div class="flex items-end gap-1 w-full justify-center">
              <span class="shrink-0">(</span>
              <span class="form-dotted min-w-[180px] px-2 text-center text-gray-700 dark:text-slate-300">{{
                coAdvisorName }}</span>
              <span class="shrink-0">)</span>
            </div>
          </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-y-10 px-4 mb-8">
            <div v-for="i in 6" :key="i" class="flex flex-col items-center break-inside-avoid">
              ...
              <div class="text-center w-[250px] text-gray-500 dark:text-slate-400">
                (........................................................)</div>
            </div>
          </div>
        </div>
...
.paper-a4 {
  ...
  min-height: 297mm;
  padding: 12mm 15mm;
  /* ลด Padding ลงเพื่อให้เนื้อหาพอดี 1 หน้า */
...
@media print {
  @page {
    size: A4 portrait;
    margin: 0cm;
    /* ตั้งค่าขอบกระดาษเป็น 0 ป้องกันเบราว์เซอร์แทรก Footer */
  }
  ...
  .paper-a4 {
    ...
    /* ขยับขอบให้เนื้อหาฟิตพอดีตอนปริ้น */
