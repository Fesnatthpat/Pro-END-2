  ...
  <div class="min-h-screen bg-[#f8f9fa] dark:bg-slate-900 py-8 px-4 ">
    <div class="max-w-[900px] mx-auto w-full">
      ...
      <div v-else class="space-y-6">
        <div v-for="report in reports" :key="report.id" class="bg-white dark:bg-slate-800 rounded-[28px] p-6 md:p-8 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow">
          ...
          <div v-if="report.feedback" class="mt-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700">
            ...
            <p class="text-sm text-slate-700 dark:text-slate-300">{{ report.feedback }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        ...
        <form @submit.prevent="handleSubmit" class="p-8 space-y-5">
          <div>
            ...
            <input v-model="newReport.title" type="text" required class="w-full px-5 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 focus:border-[#1a1a40] outline-none transition-all text-sm bg-gray-50 dark:bg-slate-800" placeholder="ระบุหัวข้อบันทึก...">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">รายละเอียดสิ่งที่แก้ไข</label>
            <textarea v-model="newReport.description" rows="4" required class="w-full px-5 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 focus:border-[#1a1a40] outline-none transition-all text-sm bg-gray-50 dark:bg-slate-800" placeholder="อธิบายรายละเอียดสิ่งที่แก้ไข..."></textarea>
          </div>
          ...
          <div class="pt-4 flex gap-3">
            <button 
              ...
              class="flex-1 px-6 py-3 rounded-full border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 font-bold text-sm hover:bg-gray-50 transition-all"
            >
              ยกเลิก
            </button>
            <button 
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div class="bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        ...
        <form @submit.prevent="handleEditSubmit" class="p-8 space-y-5">
          <div>
            <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">หัวข้อบันทึก</label>
            <input v-model="editReport.title" type="text" required class="w-full px-5 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 focus:border-[#1a1a40] outline-none transition-all text-sm bg-gray-50 dark:bg-slate-800" placeholder="ระบุหัวข้อบันทึก...">
          </div>
          <div>
            <label class="block text-sm font-bold text-gray-700 dark:text-slate-300 mb-2">รายละเอียดสิ่งที่แก้ไข</label>
            <textarea v-model="editReport.description" rows="4" required class="w-full px-5 py-3 rounded-2xl border border-gray-200 dark:border-slate-700 focus:border-[#1a1a40] outline-none transition-all text-sm bg-gray-50 dark:bg-slate-800" placeholder="อธิบายรายละเอียดสิ่งที่แก้ไข..."></textarea>
          </div>
          ...
          <div class="pt-4 flex gap-3">
            ...
            <button 
              ...
              class="flex-1 px-6 py-3 rounded-full bg-[#1a1a40] text-white font-bold text-sm hover:bg-[#2c2c54] transition-all disabled:opacity-50"
            >
              {{ submitting ? 'กำลังบันทึก...' : 'บันทึกแก้ไข' }}
            </button>
          </div>
        </form>
      </div>
