<template>
  <div
    class="flex flex-col min-h-screen bg-[#f8f9fa] dark:bg-slate-900 text-slate-800 dark:text-slate-200 transition-colors duration-300">

    <nav v-if="!hideNav"
      class="sticky top-0 z-50 bg-[#1a1a40]/95 backdrop-blur-md py-3 lg:py-4 shadow-[0_4px_25px_rgba(0,0,0,0.1)] border-b border-white/5 transition-all duration-300 print:hidden">
      <div
        class="w-full xl:max-w-[1450px] mx-auto px-4 md:px-8 flex flex-wrap lg:flex-nowrap items-center justify-between">

        <!-- 1. Logo (Left) -->
        <div class="flex-1 flex justify-start z-20">
          <NuxtLink to="/student"
            class="text-white font-extrabold text-xl flex items-center gap-3 no-underline shrink-0 group">
            <div
              class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 group-hover:scale-105 transition-transform">
              <i class="bi bi-mortarboard-fill"></i>
            </div>
            <span class="tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-indigo-200">Student
              Panel</span>
          </NuxtLink>
        </div>
        ...
        <button @click="isMenuOpen = !isMenuOpen"
          class="lg:hidden text-indigo-200 hover:text-white text-3xl focus:outline-none z-20 transition-colors">
          <i class="bi" :class="isMenuOpen ? 'bi-x' : 'bi-list'"></i>
        </button>

        <!-- Wrapper for Desktop & Mobile Menu -->
        <div
          :class="isMenuOpen ? 'max-h-[400px] opacity-100 py-4 mt-2 border-t border-white/10' : 'max-h-0 opacity-0 lg:max-h-full lg:opacity-100 lg:py-0 lg:mt-0 lg:border-none'"
          ...
          <ul class="flex flex-col lg:flex-row items-center gap-2 w-full lg:w-auto m-0 p-0">
            <li class="w-full lg:w-auto">
              <NuxtLink @click="isMenuOpen = false" to="/procedures"
            ...
            <li v-if="isApproved" class="w-full lg:w-auto">
              <NuxtLink @click="isMenuOpen = false" to="/student"
                active-class="bg-indigo-500/20 text-indigo-200 ring-1 ring-indigo-500/30"
                class="flex items-center justify-center lg:justify-start gap-2 text-slate-300 hover:text-white hover:bg-white/10 px-5 py-2.5 rounded-2xl transition-all duration-300 font-medium text-[15px] w-full whitespace-nowrap">
        ...
        <div :class="isMenuOpen ? 'flex' : 'hidden'"
          class="lg:flex flex-1 flex-col lg:flex-row justify-end items-center gap-3 w-full lg:w-auto pt-4 lg:pt-0 mt-2 lg:mt-0 border-t border-white/10 lg:border-none pb-4 lg:pb-0">

          <NuxtLink @click="isMenuOpen = false" v-if="user" to="/student/profile"
            class="flex items-center gap-3 text-white bg-white/5 hover:bg-white/10 ring-1 ring-white/10 hover:ring-white/20 transition-all duration-300 pr-4 pl-1.5 py-1.5 rounded-full w-full lg:w-auto justify-center lg:justify-start">
            <div
              class="w-8 h-8 bg-indigo-500 text-white rounded-full flex items-center justify-center font-bold text-sm shadow-sm overflow-hidden shrink-0">
            ...
            <div class="flex flex-col items-start leading-tight min-w-0">
              <span class="text-[13px] font-bold tracking-wide truncate max-w-[120px] xl:max-w-[180px]">{{ user.fullname
                }}</span>
          ...
          <button @click="handleLogout" type="button"
            class="flex items-center justify-center gap-2 text-rose-300 bg-rose-500/10 hover:bg-rose-500 hover:text-white px-5 py-2 rounded-full text-[14px] font-bold transition-all duration-300 w-full lg:w-auto shrink-0 mt-2 lg:mt-0 whitespace-nowrap">
